...
def download(job: sqlite3.Row) -> tuple[Path, dict[str, Any]]:
    ...
    cmd = ["yt-dlp", "--no-cache-dir", "--no-playlist", "--format", "bestaudio/best",
...
def process_one(con: sqlite3.Connection, job: sqlite3.Row) -> None:
    ...
    try:
        ...
        receipt = {"job_id": job["id"], "video_url": job["video_url"], "video_id": metadata.get("id") or job["video_id"], "title": metadata.get("title") or job["title"], "provider": "elevenlabs", "model": "scribe_v2", "http_status": status, "audio_duration_secs": duration, "estimated_api_cost_usd": round(float(duration or 0) / 3600 * 0.22, 4), "started_at_utc": started, "completed_at_utc": now(), "media_deleted_after_success": True}
        receipt_path.write_text(json.dumps(receipt, indent=2) + "\n")
        set_status(con, job["id"], "completed", transcript_path=str(transcript_path), receipt_path=str(receipt_path), error=None)
        print(json.dumps({"job_id": job["id"], "status": "completed", "duration": duration}))
